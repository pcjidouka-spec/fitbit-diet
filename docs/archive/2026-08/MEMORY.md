<!-- ★凍結（2026-08-08）。このファイルは **もう読まれない前提** の歴史記録である。

MEMORY.md 方式は廃止した。記憶の正本は bd（`bd remember` / `bd memories`）。
廃止理由: 知識が MEMORY.md と bd の2か所に分散し、次のセッションで片方しか読まれない。

★このプロジェクトは最終コミットが 2026-06-04 で休眠しているため、
  内容の bd への移行は **行っていない**（数か月前の作業ログで生きた知識ベースを汚さないため）。
  再開するときは、ここから必要な分だけを手で拾って `bd remember` に入れ直すこと。

経緯: capital-os-jp `docs/superpowers/specs/2026-08-08-project-template-rebuild-design.md`
-->
